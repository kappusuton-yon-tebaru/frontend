...
const fetchProjectSpaces = async (orgId: string, page: number) => {
  const response = await getData(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/resources/children/${orgId}?page=${page}&limit=8`
  );
  return {
    data: response.data,
...
const fetchRepositories = async (projectSpaceId: string, page: number) => {
  const response = await getData(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/resources/children/${projectSpaceId}?page=${page}&limit=4`
  );
  return {
    data: response.data,
    total: response.total,
